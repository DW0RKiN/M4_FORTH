...
[ ! -s "${name}.asm" ] && printf "Error: $0: \"${name}.asm\" not found!\n" >&2 && exit 2
...
! command -v pasmo >/dev/null && printf "Error: $0: pasmo not found!\n" >&2 && exit 4
...
# ------------------- Generate loader if addr exists
...
! command -v zmakebas >/dev/null && printf "Error: $0: zmakebas not found!\n" >&2 && exit 6
...
if [ ! -z "${emul}" ] ; then
    ! command -v "${emul}" >/dev/null && printf "Error: $0: ${emul} not found!\n" >&2 && exit 7
    ${emul} "$@" "${name}.tap"
    exit 0
fi
...
do
    if command -v "${emul%% *}" >/dev/null ; then
        ${emul} "${name}.tap"
        exit 0
    fi
done
