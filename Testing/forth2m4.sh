...
sed 's#^\([^;{]*\s\|^\)1+\(\s\|$\)#\1_1ADD\2#g' |
sed 's#^\([^;{]*\s\|^\)1-\(\s\|$\)#\1_1SUB\2#g' |
sed 's#^\([^;{]*\s\|^\)2+\(\s\|$\)#\1_2ADD\2#g' |
sed 's#^\([^;{]*\s\|^\)2-\(\s\|$\)#\1_2SUB\2#g' |
sed 's#^\([^;{]*\s\|^\)2\*\(\s\|$\)#\1_2MUL\2#g' |
sed 's#^\([^;{]*\s\|^\)2/\(\s\|$\)#\1_2DIV\2#g' |
sed 's#^\([^;{]*\s\|^\)0\s\+=\(\s\|$\)#\1EQ0\2#g' |
sed 's#^\([^;{]*\s\|^\)0\s\+<>\(\s\|$\)#\1NE0\2#g' |

sed 's#^\([^;{]*\s\|^\)pick\(\s\|$\)#\1PICK\2#g' |
sed 's#^\([^;{]*\s\|^\)0\s\+PICK\(\s\|$\)#\1_0_PICK\2#g' |
sed 's#^\([^;{]*\s\|^\)1\s\+PICK\(\s\|$\)#\1_1_PICK\2#g' |
sed 's#^\([^;{]*\s\|^\)2\s\+PICK\(\s\|$\)#\1_2_PICK\2#g' |
sed 's#^\([^;{]*\s\|^\)3\s\+PICK\(\s\|$\)#\1_3_PICK\2#g' |
...
sed 's#^\([^;{]*\s\|^\)2dup\(\s\|$\)#\1_2DUP\2#g' |
sed 's#^\([^;{]*\s\|^\)2drop\(\s\|$\)#\1_2DROP\2#g' |
sed 's#^\([^;{]*\s\|^\)\.s\(\s\|$\)#\1DOTS\2#g' |

sed 's#^\([^;{]*\s\|^\)\(type\)\(\s\|$\)#\1\U\2\3#g' |
sed 's#^\([^;{]*\s\|^\)\(emit\)\(\s\|$\)#\1\U\2\3#g' |
sed 's#^\([^;{]*\s\|^\)\(key\)\(\s\|$\)#\1\U\2\3#g' |
...
sed 's#^\([^;{]*\s\|^\)1\s\+SUB\(\s\|$\)#\1_1SUB\2#g' |
sed 's#^\([^;{]*\s\|^\)2\s\+SUB\(\s\|$\)#\1_2SUB\2#g' |
sed 's#^\([^;{]*\s\|^\)1\s\+ADD\(\s\|$\)#\1_1ADD\2#g' |
sed 's#^\([^;{]*\s\|^\)2\s\+ADD\(\s\|$\)#\1_2ADD\2#g' |
...
sed 's#^\([^;{]*\s\|^\)_2DUP\sTYPE\(\s\|$\)#\1_2DUP_TYPE\2#g' |

sed 's#^\([^;{]*\s\|^\)_2DUP\s\+ULT\s\+IF\(\s\|$\)#\1_2DUP_ULT_IF\2#g' |
sed 's#^\([^;{]*\s\|^\)_2DUP\s\+ULE\s\+IF\(\s\|$\)#\1_2DUP_ULE_IF\2#g' |
sed 's#^\([^;{]*\s\|^\)_2DUP\s\+UGT\s\+IF\(\s\|$\)#\1_2DUP_UGT_IF\2#g' |
sed 's#^\([^;{]*\s\|^\)_2DUP\s\+UGE\s\+IF\(\s\|$\)#\1_2DUP_UGE_IF\2#g' |
...
sed 's#^\([^;{]*\s\|^\)\([+]*[0-9]\+\)\s\+MUL\(\s\|$\)#\1PUSH_MUL(\2)\3#g' > $TMPFILE2
...
while :
do
    cat $TMPFILE | sed 's#^\([^;{]*\s\|^\)\([-+]*[0-9]\+\)\s\+\([-+]*[0-9]\+\)\(\s\|$\)#\1PUSH2(\2,\3)\4#g' > $TMPFILE2
    diff $TMPFILE $TMPFILE2 > /dev/null 2>&1
    error=$?
    cat $TMPFILE2 > $TMPFILE
    [ $error -gt 1 ] && exit
