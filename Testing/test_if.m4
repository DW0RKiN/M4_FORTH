...
; ^^^^^
    ...
    PRINT_I({0x0D, "R.A.S:"}) RAS UDOT CR
    STOP
    
COLON(x_x_test)
    ; signed
     _2DUP EQ IF PRINT_I( {"=" }) THEN
     _2DUP_EQ_IF PRINT_I( {"=" }) THEN
     _2DUP EQ_IF PRINT_I( {"=,"}) THEN
     _2DUP NE IF PRINT_I({"<>" }) THEN
     _2DUP_NE_IF PRINT_I({"<>" }) THEN
     _2DUP NE_IF PRINT_I({"<>,"}) THEN
     _2DUP LT IF PRINT_I( {"<" }) THEN
     _2DUP_LT_IF PRINT_I( {"<" }) THEN
     _2DUP LT_IF PRINT_I( {"<,"}) THEN
     _2DUP LE IF PRINT_I({"<=" }) THEN
     _2DUP_LE_IF PRINT_I({"<=" }) THEN
     _2DUP LE_IF PRINT_I({"<=,"}) THEN
     _2DUP GT IF PRINT_I( {">" }) THEN
     _2DUP_GT_IF PRINT_I( {">" }) THEN
     _2DUP GT_IF PRINT_I( {">,"}) THEN
     _2DUP GE IF PRINT_I({">=" }) THEN
     _2DUP_GE_IF PRINT_I({">=" }) THEN
     _2DUP GE_IF PRINT_I({">=,"}) THEN
    OVER DOT DUP SPACE_DOT CR  
    ; unsigned
    _2DUP UEQ IF PRINT_I( {"=" }) THEN
    _2DUP_UEQ_IF PRINT_I( {"=" }) THEN
    _2DUP UEQ_IF PRINT_I( {"=,"}) THEN
    _2DUP UNE IF PRINT_I({"<>" }) THEN
    _2DUP_UNE_IF PRINT_I({"<>" }) THEN
    _2DUP UNE_IF PRINT_I({"<>,"}) THEN
    _2DUP ULT IF PRINT_I( {"<" }) THEN
    _2DUP_ULT_IF PRINT_I( {"<" }) THEN
    _2DUP ULT_IF PRINT_I( {"<,"}) THEN
    _2DUP ULE IF PRINT_I({"<=" }) THEN
    _2DUP_ULE_IF PRINT_I({"<=" }) THEN
    _2DUP ULE_IF PRINT_I({"<=,"}) THEN
    _2DUP UGT IF PRINT_I( {">" }) THEN
    _2DUP_UGT_IF PRINT_I( {">" }) THEN
    _2DUP UGT_IF PRINT_I( {">,"}) THEN
    _2DUP UGE IF PRINT_I({">=" }) THEN
    _2DUP_UGE_IF PRINT_I({">=" }) THEN
    _2DUP UGE_IF PRINT_I({">=,"}) THEN
